...
add_action('add_attachment', function($post_ID) {
    if (wp_attachment_is_image($post_ID)) {
        ...
        // Use DateTimeOriginal (created_timestamp) from EXIF if available, otherwise use current time.
        $image_time = !empty($meta['created_timestamp'])
            ? date('Y-m-d H.i.s', $meta['created_timestamp'])
...
/**
 * Convert Markdown (even mixed with HTML) to HTML on save,
 * and store the original Markdown in post meta for 'post' and 'page'.
...
add_filter('wp_insert_post_data', function($data, $postarr) {
    ...
    if (function_exists('has_blocks') && has_blocks($data['post_content'])) {
        // Clear any stale markdown meta, since we're now using blocks.
...
/* ============================================================
   6. Google Analytics 4 Tracking (Best Practice)
   ============================================================ */

/**
 * Output GA4 tracking code, excluding local/dev/staging and logged-in users.
 */
add_action('wp_head', function() {
    // Defensive check for HTTP_HOST.
    ...
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-04C3HJVNTD"></script>
    <script>
      window.dataLayer = window.dataLayer || [];
      function gtag(){dataLayer.push(arguments);}
      gtag('js', new Date());
      gtag('config', 'G-04C3HJVNTD');
    </script>
    <!-- End Google tag -->
    <?php
});
